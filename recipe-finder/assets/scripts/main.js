...
function getRandomIngredient() {
  let ingredient =
    possibleFoodIngredients[
      Math.floor(Math.random() * possibleFoodIngredients.length)
    ];
  ...
  fetch(foodURL + ingredient + appIDKey + "&random=true")
    ...
    .then(function (data) {
      if (data.count !== 0) {
        ...
        let randomFromReturned =
          allReturnedRecipes[
            Math.floor(Math.random() * allReturnedRecipes.length)
          ];
